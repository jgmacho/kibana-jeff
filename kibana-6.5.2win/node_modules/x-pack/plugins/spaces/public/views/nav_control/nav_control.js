...
module.controller('spacesNavController', ($scope, $http, chrome, Private, activeSpace) => {
    ...
    $scope.$parent.$watch('isVisible', function isVisibleWatcher(isVisible) {
        if (isVisible && !mounted && !pathProvider.isUnauthenticated()) {
            react_dom_2.render(react_1.default.createElement(nav_control_popover_1.NavControlPopover, { spacesManager: spacesManager, activeSpace: activeSpace, userProfile: userProfile, anchorPosition: 'rightCenter', buttonClass: spaces_global_nav_button_1.SpacesGlobalNavButton }), domNode);
            mounted = true;
        }
    });
    ...
    $scope.$on('$destroy', () => {
        if (domNode) {
            react_dom_2.unmountComponentAtNode(domNode);
        }
        mounted = false;
    });
});
...
chrome_header_nav_controls_1.chromeHeaderNavControlsRegistry.register(($http, chrome, Private, activeSpace) => ({
    ...
    render(el) {
        ...
        const spaceSelectorURL = chrome.getInjected('spaceSelectorURL');
        spacesManager = new spaces_manager_1.SpacesManager($http, chrome, spaceSelectorURL);
        react_dom_1.default.render(react_1.default.createElement(nav_control_popover_1.NavControlPopover, { spacesManager: spacesManager, activeSpace: activeSpace, userProfile: userProfile, anchorPosition: "downLeft", buttonClass: spaces_header_nav_button_1.SpacesHeaderNavButton }), el);
    },
}));
