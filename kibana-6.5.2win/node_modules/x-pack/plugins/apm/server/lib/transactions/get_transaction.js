...
async function getTransaction(transactionId, traceId, setup) {
    ...
    const result = ts_optchain_1.oc(resp).hits.hits[0]._source();
    if (result === undefined) {
        const notFoundError = new Error(`No results found for transaction ID ${transactionId} and trace ID ${traceId}`);
