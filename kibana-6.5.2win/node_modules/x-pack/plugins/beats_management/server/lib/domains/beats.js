...
class CMBeatsDomain {
    ...
    async enrollBeat(enrollmentToken, beatId, remoteAddress, beat) {
        ...
        if (expires_on && moment_1.default(expires_on).isBefore(moment_1.default())) {
            return { status: lib_1.BeatEnrollmentStatus.ExpiredEnrollmentToken };
        }
        if (!token) {
            return { status: lib_1.BeatEnrollmentStatus.InvalidEnrollmentToken };
        }
        const existingBeat = await this.getById(this.framework.internalUser, beatId);
        if (existingBeat) {
            return { status: lib_1.BeatEnrollmentStatus.Success };
        }
        ...
        return { status: lib_1.BeatEnrollmentStatus.Success, accessToken };
    }
